...
func loadAttachments(ctx context.Context, deps *DetailViewDeps, id string, pageData *PageData) {
	...
	if deps.ListAttachments != nil {
		resp, err := deps.ListAttachments(ctx, "workspace", id)
		if err != nil {
			log.Printf("Failed to list attachments for workspace %s: %v", id, err)
		}
...
// attachmentConfig builds the shared attachment.Config for the workspace entity.

// Package model holds the shared view-models and status helpers for the
// conversation surface (Plan-4, 2026-06-03). It is a LEAF package: it imports
// only entydad (label types) + esqyma proto, never the conversation view
// sub-packages — this breaks the import cycle that would otherwise arise from
// both the root conversation package and its list/detail/action/portal
// sub-packages needing these helpers.
package model
...
func StatusLabel(s conversationpb.ConversationStatus, l entydad.ConversationStatusLabels) string {
	switch s {
...
type PostsPartialData struct {
	Posts  []PostBubble
	Labels entydad.ConversationPostLabels
}

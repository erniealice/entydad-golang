...
func loadAttachments(ctx context.Context, deps *DetailViewDeps, id string, pageData *PageData) {
	...
	if deps.ListAttachments != nil {
		resp, err := deps.ListAttachments(ctx, "workspace_user", id)
		if err != nil {
			log.Printf("Failed to list attachments for workspace_user %s: %v", id, err)
		}
...
// attachmentConfig builds the shared attachment.Config for the workspace_user entity.

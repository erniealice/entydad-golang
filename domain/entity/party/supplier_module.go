...
import (
	...
	suppliertagpkg "github.com/erniealice/entydad-golang/domain/entity/party/supplier_tag"
	supplieraction "github.com/erniealice/entydad-golang/domain/entity/party/supplier/action"
	supplierdashboard "github.com/erniealice/entydad-golang/domain/entity/party/supplier/dashboard"
	supplierdetail "github.com/erniealice/entydad-golang/domain/entity/party/supplier/detail"
	supplierform "github.com/erniealice/entydad-golang/domain/entity/party/supplier/form"
	supplierlist "github.com/erniealice/entydad-golang/domain/entity/party/supplier/list"
...
// SupplierPaymentTermOption is re-exported from form for use by callers wiring SupplierModuleDeps.
type SupplierPaymentTermOption = supplierform.PaymentTermOption

// SupplierModuleDeps holds all dependencies for the supplier module.
type SupplierModuleDeps struct {
	Routes entitysupplier.Routes
	...
	Labels               entitysupplier.Labels
	DashboardLabels      entitysupplier.DashboardLabels
	...
	ListPaymentTerms func(ctx context.Context) ([]*SupplierPaymentTermOption, error)
	...
	// Cross-entity (DAG): direct sibling type, not via the facade/root.
	SupplierTagRoutes suppliertagpkg.Routes
...
// SupplierModule holds all constructed supplier views.
...
func NewSupplierModule(deps *SupplierModuleDeps) *SupplierModule {
...
// supplierRouteRegistrarFull extends view.RouteRegistrar with HandleFunc support
// for raw http.HandlerFunc routes (e.g., CSV exports).
type supplierRouteRegistrarFull interface {
	view.RouteRegistrar
	HandleFunc(method, path string, handler http.HandlerFunc, middlewares ...string)
}

// supplierHandleFunc is a nil-safe helper that registers an http.HandlerFunc route if
// the RouteRegistrar supports it, otherwise logs a warning and skips.
func supplierHandleFunc(r view.RouteRegistrar, method, path string, handler http.HandlerFunc) {
	if handler == nil {
		return
	}
	if full, ok := r.(supplierRouteRegistrarFull); ok {
...
func (m *SupplierModule) RegisterRoutes(r view.RouteRegistrar) {
	...
	supplierHandleFunc(r, "GET", m.routes.StatementExportURL, m.StatementExport)
}

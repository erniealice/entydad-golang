...
func supplierColumns(l entitysupplier.Labels) []types.TableColumn {
...
func buildTableRows(suppliers []*supplierpb.Supplier, status string, l entitysupplier.Labels, sl entydad.SharedLabels, cl pyeza.CommonLabels, routes entitysupplier.Routes, inUseIDs map[string]bool, balances map[string]int64, perms *types.UserPermissions) []types.TableRow {
	rows := []types.TableRow{}
...
func statusPageCaption(l entitysupplier.Labels, status string) string {
	switch status {
	case "active":
...
func statusEmptyTitle(l entitysupplier.Labels, status string) string {
	switch status {
	case "active":
...
func statusEmptyMessage(l entitysupplier.Labels, status string) string {
	switch status {
	case "active":
...
func supplierTransitionLabels(target string, l entitysupplier.Labels, sl entydad.SharedLabels) (string, string, string) {
	switch target {
	case "active":
...
func buildRowActions(id, name, status string, isInUse bool, l entitysupplier.Labels, sl entydad.SharedLabels, cl pyeza.CommonLabels, routes entitysupplier.Routes, perms *types.UserPermissions) []types.TableAction {
...
func buildBulkActions(l entitysupplier.Labels, sl entydad.SharedLabels, cl pyeza.CommonLabels, status string, routes entitysupplier.Routes, perms *types.UserPermissions) []types.BulkAction {

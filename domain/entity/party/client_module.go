...
import (
	...
	clientaction "github.com/erniealice/entydad-golang/domain/entity/party/client/action"
	clientdashboard "github.com/erniealice/entydad-golang/domain/entity/party/client/dashboard"
	...
	clientlist "github.com/erniealice/entydad-golang/domain/entity/party/client/list"
...
// ClientPaymentTermOption is re-exported from form for use by callers wiring ClientModuleDeps.
type ClientPaymentTermOption = clientform.PaymentTermOption

// ClientModuleDeps holds all dependencies for the client module.
type ClientModuleDeps struct {
	...
	DashboardLabels             entityclient.DashboardLabels
	...
	// ListClientPriceSchedules fetches PriceSchedules scoped to a client_id for
	// the PriceSchedules tab. Wired from centymo via the centymo block;
	// nil-safe (tab renders empty state when not wired).
	ListClientPriceSchedules func(ctx context.Context, clientID string) ([]clientdetail.ClientPriceScheduleRow, error)

	// PriceScheduleAddURL is the centymo PriceSchedule-add drawer URL; the
...
// ClientModule holds all constructed client views.
...
func NewClientModule(deps *ClientModuleDeps) *ClientModule {
...
// clientRouteRegistrarFull extends view.RouteRegistrar with HandleFunc support
// for raw http.HandlerFunc routes (e.g., CSV exports).
type clientRouteRegistrarFull interface {
	view.RouteRegistrar
	HandleFunc(method, path string, handler http.HandlerFunc, middlewares ...string)
}

// clientHandleFunc is a nil-safe helper that registers an http.HandlerFunc route if
// the RouteRegistrar supports it, otherwise logs a warning and skips.
func clientHandleFunc(r view.RouteRegistrar, method, path string, handler http.HandlerFunc) {
	if handler == nil {
		return
	}
	if full, ok := r.(clientRouteRegistrarFull); ok {
...
func (m *ClientModule) RegisterRoutes(r view.RouteRegistrar) {
	...
	clientHandleFunc(r, "GET", m.routes.StatementExportURL, m.StatementExport)

...
func NewView(deps *ListViewDeps) view.View {
	return view.ViewFunc(func(ctx context.Context, viewCtx *view.ViewContext) view.ViewResult {
		if !view.GetUserPermissions(ctx).Can("location_area", "list") {
			return view.Forbidden("location_area:list")
		}
...
func buildTableConfig(ctx context.Context, deps *ListViewDeps, columns []types.TableColumn, status string, p tableparams.TableQueryParams) (*types.TableConfig, error) {
	...
	tableConfig := &types.TableConfig{
		...
		PrimaryAction: &types.PrimaryAction{
			...
			DisabledTooltip: fmt.Sprintf(deps.CommonLabels.Errors.MissingPermission, "location_area:create"),
		},
		BulkActions:      &bulkCfg,
		ServerPagination: sp,
	}
...
func locationAreaColumns(l locationarea.Labels) []types.TableColumn {
...
func buildTableRows(items []*LocationAreaItem, status string, l locationarea.Labels, sl entydad.SharedLabels, routes locationarea.Routes, inUseIDs map[string]bool, perms *types.UserPermissions) []types.TableRow {
	rows := []types.TableRow{}
...
func statusTitle(l locationarea.Labels, status string) string {
	switch status {
	case "active":
...
func statusSubtitle(l locationarea.Labels, status string) string {
	switch status {
	case "active":
...
func statusEmptyTitle(l locationarea.Labels, status string) string {
	switch status {
	case "active":
...
func statusEmptyMessage(l locationarea.Labels, status string) string {
	switch status {
	case "active":
...
func buildBulkActions(l locationarea.Labels, sl entydad.SharedLabels, common pyeza.CommonLabels, status string, routes locationarea.Routes, perms *types.UserPermissions) []types.BulkAction {
	actions := []types.BulkAction{}

	canUpdate := perms.Can("location_area", "update")
	canDelete := perms.Can("location_area", "delete")
	...
	case "active":
		actions = append(actions, types.BulkAction{
			...
			Disabled:        !canUpdate,
			DisabledTooltip: fmt.Sprintf(common.Errors.MissingPermission, "location_area:update"),
		})
	case "inactive":
		actions = append(actions, types.BulkAction{
			Key:             "activate",
	...
	actions = append(actions, types.BulkAction{
		...
		Disabled:         !canDelete,
		DisabledTooltip:  fmt.Sprintf(common.Errors.MissingPermission, "location_area:delete"),
	})

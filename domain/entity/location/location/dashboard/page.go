...
type Deps struct {
	...
	Dashboard       location.DashboardLabels
	...
	LocationAreaRoutes locationarea.Routes
...
func buildTopAreasHTML(rows []LocationAreaCount, l location.DashboardLabels) template.HTML {
	if len(rows) == 0 {
...
func buildRecentList(locations []*locationpb.Location, l location.DashboardLabels) []types.ActivityItem {
	if len(locations) == 0 {
		return nil
	}

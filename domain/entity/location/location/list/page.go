...
func locationColumns(l location.Labels) []types.TableColumn {
...
func buildTableRows(locations []*locationpb.Location, status string, l location.Labels, sl entydad.SharedLabels, routes location.Routes, inUseIDs map[string]bool, perms *types.UserPermissions) []types.TableRow {
	rows := []types.TableRow{}
...
func statusTitle(l location.Labels, status string) string {
	switch status {
	case "active":
...
func statusSubtitle(l location.Labels, status string) string {
	switch status {
	case "active":
...
func statusEmptyTitle(l location.Labels, status string) string {
	switch status {
	case "active":
...
func statusEmptyMessage(l location.Labels, status string) string {
	switch status {
	case "active":
...
func buildBulkActions(l location.Labels, sl entydad.SharedLabels, common pyeza.CommonLabels, status string, routes location.Routes, perms *types.UserPermissions) []types.BulkAction {

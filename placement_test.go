...
var crossCutting = false // entydad is a domain package (entity domain + tax/communication views).
// legacyAllow — each entry carries a dated EXPIRES stamp (no open-ended
// exemptions). Empty = STRICT (the capstone target). entydad realizes Option B as
// domain/entity/<subcontext>/<entity>/ (subcontext = navigation-only folder per
// option-b-adoption.md §entydad note). The R2′ gate now MODELS that sub-context
// layer natively (subContexts config + one-level recursion), so the former
// commerce/identity/party "template-gap" excuses are GONE. The genuine residuals
// are: the root A-era stubs (R1), the un-relocated planning/views surfaces, and
// the two entydad-local tag entities under party/ that have no esqyma proto entity.
var legacyAllow = map[string]string{
	...
	// Now that R2′ recurses into the chartered sub-contexts (subContexts below), these
	// two grandchildren are validated as entities of `entity` and flagged. They are
	// real residuals — not a template-logic gap — so they carry precise full-path keys
	// (the basename form would never over-excuse a same-named dir elsewhere).
	"domain/entity/party/client_tag":   "entydad-local tag entity, no 1:1 esqyma proto entity — EXPIRES 2026-07-15 (capstone: add the esqyma client_tag entity or fold into client/ as a sub-view)",
	"domain/entity/party/supplier_tag": "entydad-local tag entity, no 1:1 esqyma proto entity — EXPIRES 2026-07-15 (capstone: add the esqyma supplier_tag entity or fold into supplier/ as a sub-view)",
}
var charterViews = []string{} // crossCutting only — unused here
// subContexts: entydad's CHARTERED Option-B shape is domain/entity/<subcontext>/<entity>/
// where <subcontext> ∈ {commerce,identity,location,party} is a NAVIGATION-ONLY folder
// (option-b-adoption.md §entydad note: "the sub-context becomes a navigation folder
// only"). Each sub-context dir is NOT itself an esqyma entity — R2′ recurses ONE level
// into it and validates each child as an entity of `entity`. This is the ONLY non-empty
// subContexts in the fleet; every other package keeps it empty (no-op).
var subContexts = []string{"commerce", "identity", "location", "party"}
